...
- **강제 캐시 파기(Cache-Busting) 아키텍처:** Vercel 배포 후에도 카카오톡 등 인앱 브라우저가 스크립트를 재탕(Zombie Cache)하여 렌더링 결함이 발생하는 것을 영구 차단하기 위해, `index.html` 의 정적 리소스 호출부(`app.js?v=v6`) 및 `sw.js?v=v6` 에 Query Parameter를 부여하여 브라우저가 항상 최신 버전을 다운로드하도록 강제함.
- **서버단 RichText 파서 + 이원화 렌더링 아키텍처:** 구글 시트의 색상 서식(문법 강조)을 살리기 위해 서버에서 `.getRichTextValues()`로 폰트 색상을 HTML `<span>` 태그로 파싱해 전달함. 화면 렌더링(HTML)과 TTS 오디오 엔진(Plain Text)에 각각 분리 주입하여 디자인과 오디오 엔진의 충돌을 원천 차단함.
- **SWR (Stale-While-Revalidate) & Zero-Loading 아키텍처:** 구글 API(GAS)의 심각한 콜드 스타트 응답 지연을 회피하기 위해, 앱 실행 시 로컬 스토리지(`localStorage`)에 저장된 캐시 데이터로 즉시 화면을 그려 대기시간을 0초로 만듦. 이후 백그라운드에서 조용히 서버와 대조 작업을 수행함.
- **Background Data Sweeper & Hot Reload (비동기 감찰):** 앱이 백그라운드에서 포그라운드로 복귀 시(`visibilitychange`) 비동기로 서버와 교신하여 데이터 변동을 감지. 변경 시 부드러운 '토스트 플로팅 알림'을 띄우고, 사용자 터치 시 화면 깜빡임이나 스크롤 유실 없이 최신 데이터를 즉시 덮어쓰는 무중단 업데이트 구현.
...
- **배포 후 스크립트(.js) 미반영 현상 (카카오톡 좀비 캐시):** 일반적인 PWA `Network-First` 설계에도 불구하고 카카오톡 인앱 브라우저는 새로고침 시 HTML만 재요청하고 내부 JS/CSS는 캐시를 악착같이 강제 재사용함. 이로 인해 업데이트가 무시되는 문제를 뚫기 위해 Cache-Busting 꼬리표(`?v=v6`)를 스크립트에 강제 부여함.
...
- **구글 시트 폰트 색상 증발 현상:** 단순 `.getValues()` 파싱은 모든 서식 메타데이터를 파괴하고 1차원 평문 버퍼만 반환함. 이로 인해 언어 학습의 핵심인 인지 부각(Syntax highlighting)이 소실되었던 문제를 구조적으로 개선함.
- **모바일 생명주기(Lifecycle)로 인한 PWA Data Stale 방어:** 모바일 유저의 대부분은 앱을 완전히 끄지 않고 홈 화면으로 숨겨둠(Suspend). 이로 인해 `DOMContentLoaded`에만 의존하던 기존 데이터 로딩 방식은 며칠 전의 낡은 스냅샷만을 보여주게 됨. 데이터 수동 동기화를 위해 사용자의 새로고침 행위를 강제하는 최악의 UX를 탈피하고자 Background Sync 체계를 도입함.
- **구글 서버 (GAS) 접속 시 무한 로딩(병목) 증상:** 구글 Apps Script 특유의 V8 엔진 Cold Start 딜레이와 낡은 네트워크 환경의 사용자에게서 데이터 통신이 수 초 이상 멈추는 Blocking 현상이 발생함. 매번 전체 데이터를 새로 통신하는 하수 설계의 한계점을 극복하기 위해 로컬 캐시 기반 SWR 패턴을 도입하여 해결함.
